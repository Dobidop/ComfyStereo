...
class StereoImageNode:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "fill_depth_threshold": ("FLOAT", {"default": 0.3, "min": 0.0, "max": 1.0, "step": 0.01, "tooltip": "Gap-fill bg-eligibility threshold (normalized depth, 0=far/bg, 1=near/fg). Pixels with rendered depth below this value are considered valid bg sources for filling disocclusion gaps. Lower = stricter — excludes mid-depth pixels at the fg/bg boundary that may carry fg-tinted color from stretched-mesh interpolation. Raise toward 0.5 if gaps look unfilled; lower toward 0.2 if you see a thin 'translated fg edge' smear next to foreground silhouettes."}),
                "edge_setback": ("INT", {"default": 8, "min": 0, "max": 32, "step": 1, "tooltip": "How far (in pixels) the gap fill steps past the nearest bg-eligible pixel before sampling color. Helps skip the antialiased / barycentric-interpolated transition zone at the fg/bg boundary, where the color is often fg-tinted even when the depth reads as bg. Raise to push sampling deeper into clean bg; lower if fill looks too uniform or smears across thin bg features."}),
    ...
    def generate(self, image, depth_map, divergence, separation, modes,
                 stereo_balance, convergence_point, stereo_offset_exponent, fill_technique, depth_blur_edge_threshold, depth_blur_strength, depth_map_blur, depth_blur_falloff=1.0, depth_blur_vert_smooth=0, skip_flat_depth=False, flat_depth_threshold=1.5, fill_depth_threshold=0.3, edge_setback=8, batch_size=4):
